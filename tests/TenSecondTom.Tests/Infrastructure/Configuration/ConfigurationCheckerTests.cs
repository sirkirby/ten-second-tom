...
public sealed class ConfigurationCheckerTests
{
    ...
    private readonly Mock<ITemplateInstaller> _mockTemplateInstaller;

    public ConfigurationCheckerTests()
    {
        _mockLogger = new Mock<ILogger<ConfigurationChecker>>();
        _mockTemplateInstaller = new Mock<ITemplateInstaller>();
        _mockTemplateInstaller
            .Setup(x => x.InstallDefaultsAsync(
                It.IsAny<string>(),
                It.IsAny<bool>(),
                It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result<TemplateInstallationResult>.Success(new TemplateInstallationResult
            {
